...
from roboclaw.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from roboclaw.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("roboclaw.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "roboclaw.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("roboclaw.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "roboclaw.channels.matrix.asyncio.create_task", _fake_create_task
    )
